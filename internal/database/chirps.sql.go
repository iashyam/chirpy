...
const getAuthorChrips = `-- name: GetAuthorChrips :many
SELECT id, created_at, updated_at, body, user_id from chirps
WHERE user_id=$1
ORDER by created_at ASC
`

func (q *Queries) GetAuthorChrips(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAuthorChrips, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getChipByID = `-- name: GetChipByID :one
SELECT id, created_at, updated_at, body, user_id FROM chirps
WHERE id=$1
`

func (q *Queries) GetChipByID(ctx context.Context, id uuid.UUID) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, getChipByID, id)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
